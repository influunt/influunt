...
create table areas_new (
  ...
  data_criacao                  timestamp not null,
  data_atualizacao              timestamp not null,
  constraint pk_areas_new primary key (id)
);
...
alter table areas_new add constraint fk_areas_new_cidade_id foreign key (cidade_id) references cidades (id) on delete restrict on update restrict;
create index ix_areas_new_cidade_id on areas_new (cidade_id);
