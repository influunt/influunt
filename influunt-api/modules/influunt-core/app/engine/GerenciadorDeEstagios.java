...
public class GerenciadorDeEstagios  implements EventoCallback{
    ...
    private void geraIntervalos(Integer index) {
        ...
        final long tempoVerde = estagioPlano.getTempoVerde() * 1000L;
    ...
    private void processarDetectorVeicular(EventoMotor eventoMotor) {
        ...
        if(estagio.isDemandaPrioritaria() && !estagioPlanoAtual.getEstagio().equals(estagio)){
            ...
            if(!proximoEstagio) {
                reduziTempoEstagioAtual(estagioPlanoAnterior);
    ...
    private void reduziTempoEstagioAtual(EstagioPlano estagioPlanoAnterior) {
        Map.Entry<Range<Long>, Intervalo> range = this.intervalos.getEntry(contador);
